...
client_id = '3ecb1f2e-8ee3-4e0d-a7b1-fc7a10612874'
redirect_uri = 'http://localhost:7002/api/callback'
authEndPoint = 'https://fhir.epic.com/interconnect-fhir-oauth/oauth2/authorize'
tokenEndPoint = 'https://fhir.epic.com/interconnect-fhir-oauth/oauth2/token'  # Correct variable name
scope = 'profile openid email'
state = os.urandom(16).hex()

email_app.secret_key = 'advnugdsfcnuSGDUFDH0rgfnvwhurf'
# CORS(email_app, supports_credentials=True)
# CORS(email_app, origins=['http://localhost:5000'])
# CORS(email_app, resources={r"/api/*": {"origins": "http://localhost:5000"}})
...
@email_app.route("/getAuthUrl")
def get_auth_url():    
    auth_url = (f'{authEndPoint}?client_id={client_id}'
...
@email_app.route("/api/callback", methods=["GET", "POST"])
def callback():
...
@email_app.route('/api/get_access_token', methods=['GET'])
def get_access_token():
    access_token = session.get('access_token')
    print(f'acesss is  token {access_token}')
...
def practitioner_details():
    ...
    response = requests.get(f"https://fhir.epic.com/interconnect-fhir-oauth/api/FHIR/R4/Practitioner/{practitioner_id}", headers=headers)
